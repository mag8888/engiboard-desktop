## [v0.1.188] — 2026-07-24 — Fix: растягивание задачи сломалось после v0.1.187

Регресс от моего же фикса #4 (ресайз строки). Оба дефекта давали эффект
«пропала функция растягивать таску»:

- Fix: простой КЛИК по ручке ресайза (без перетаскивания) писал в t.h
  `parseInt('')` === NaN и оставлял на строке класс `.row-resized` БЕЗ высоты —
  строка превращалась в пустую растянутую сетку (grid-стретч без размера).
  Теперь без реального движения мыши ничего не сохраняем и снимаем класс.

- Fix: два быстрых растягивания подряд ОС отдаёт как `dblclick`, из-за чего
  срабатывал добавленный в v0.1.187 сброс высоты и молча стирал только что
  заданный размер. Сброс теперь не срабатывает сразу после драга (флаг
  `_resizeMoved`); осознанный двойной клик без драга сбрасывает как и прежде.

Regression R32 закрывает оба случая: голый клик — no-op, драг переживает
dblclick, намеренный сброс работает. Всего 32 regression + 15 smoke + 6 editor
= 53 проверки зелёные.
...
UI-правки со встречи (P1, «до запуска в работу»):

- Fix (#9): стрелки перемещения ↑/↓ и drag-ручку сделали крупнее (колонка ручки
  30→40px, стрелки 22×18→30×24, svg 13→17, ручка 14→20px).
- Fix (#10): кнопки Presentation/Link — наверх карточки (через flex order), чат
  занял полезное место снизу. Regression R21.
- Fix (#11): неделя-бар контрастнее (фон .07→.14, рамка-акцент + левый бар) и
  «папочный» отступ — задачи и add-row внутри недели сдвинуты вправо. R22.
- Fix (#12): per-project счётчики на чипах статусов (All + каждый статус). R23.
- Fix (#13): постоянный плейсхолдер «+ Add week» внизу списка недель — паттерн
  как у «+ Add task» (клик → модалка создания недели). R24.

Regression-набор: 24 проверки (R1-R24) + 15 smoke зелёные локально; CI на каждый push.
Все 7 P1-багов + 5 P1-UI со встречи 2026-07-17 закрыты.

Приоритет 2 со встречи (по возможности) — сделано:

- #14 Collapse All / Expand All недель одной кнопкой (R25).
- #15 Undo/Redo: видимые кнопки в тулбаре (гаснут при пустом стеке) + расширили
  покрытие (создание/удаление недели); стек 50, Cmd/Ctrl+Z / +Shift+Z (R26).
- #16 Presence «кто в проекте» — проверено: реализовано и подключено (board+task
  каналы, аватарки в шапке и на строках), деградирует чисто офлайн; живой тест с
  2 аккаунтов — на стороне Дмитрия (R27).
- #17 Чат: редактирование/удаление отдельных сообщений (с undo) + подсветка
  @упоминаний; общий рендер для строки и презентации (R28). Пуш-уведомление о
  теге требует облачного слоя уведомлений — вынесено в бэклог.
- #18 Длинные ссылки в чате показываются сокращённо (полный URL в href/title) (R29).
- #19 Толщина линии — вертикальный левый рейл в редакторе, «как в Paint» (E6).
- #21 Выноска: текст со стрелкой-лидером, редактируемая подпись (E5).
- #22 Полилиния: клик-по-точкам, финиш dbl-click/Enter/Esc (E3/E4).
- #23 Dimension вернули (ручная подпись, напр. «30 mm») (E1/E2).
- #24 Презентация: номер задачи жирным/крупнее, описание мельче (R30).
- #25 Новый проект: поле «Language» — понятная выпадашка вместо свободного ввода (R31).

Не трогали (Дмитрий на созвоне просил отложить): #20 текст редактируемый после
постановки («сюда не полезем»), #26 метки как объекты vs печать, #27 лимит карусели.

Regression: 31 (R1-R31) + 15 smoke + 6 editor (E1-E6) = 52 проверки зелёные.
